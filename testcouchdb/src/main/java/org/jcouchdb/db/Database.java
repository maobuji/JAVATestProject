...
public class Database
{
    ...
    private static final String FIND_DOCUMENT_INFIX = "find";
    ...
    protected static Logger log = LoggerFactory.getLogger(org.jcouchdb.db.Database.class);
    ...
    public Response mangoQuery(String mangoQueryJson) throws Exception{
        Response resp = null;

            resp = server.post("/" + name + "/_"+FIND_DOCUMENT_INFIX, mangoQueryJson);
            if (resp.getCode() == 409) {
                throw new UpdateConflictException("error creating document " + mangoQueryJson + "in database '" + name + "'", resp);
            } else if (resp.getCode() == 403) {
                throw new DocumentValidationException(resp);
            } else if (!resp.isOk()) {
                throw new DataAccessException("error creating document " + mangoQueryJson + "in database '" + name + "'", resp);
            }
            return resp;
    ...
    {
        this(new org.jcouchdb.db.ServerImpl(host), name);
    }
